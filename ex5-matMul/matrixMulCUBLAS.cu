...
/*
   nvcc -O2 -gencode arch=compute_75,code=sm_75 -gencode arch=compute_80,code=sm_80 -o test_cublas -L /software/cuda-11.5-el8-x86_64/lib64 -lcuda -lcublas matrixMulCUBLAS.cu
   ./test 6400
*/
...
/* Host implementation of a simple version of sgemm 
    C = alpha * A * B + beta * C
   All the matrices are of size (n x n)
*/
...
int main(int argc, char **argv)
{
  ...
  // store
  h_C_ref = h_C;
  float msecCPUTotal = (float)(clock() - start)/CLOCKS_PER_SEC;

  double flopsPerMatrixMul = 2.0 * static_cast<double>(N) *
                             static_cast<double>(N) *
                             static_cast<double>(N) + static_cast<double>(N);
  double gigaFlops =
      (flopsPerMatrixMul * 1.0e-9f) / (msecCPUTotal / 1000.0f);
  printf(
      "Performance= %.2f GFlop/s, Time= %.3f msec\n",
      gigaFlops, msecCPUTotal);


  /* Allocate (new) host memory for reading back the result from device memory */
  h_C = reinterpret_cast<float *>(malloc(n2 * sizeof(h_C[0])));
  ...
  printf("Computing result using GPU using CUBLAS...\n");
  start = clock();
  ...
  float msecGPUTotal = (float)(clock() - start)/CLOCKS_PER_SEC;
  flopsPerMatrixMul = 2.0 * static_cast<double>(N) *
                            static_cast<double>(N) *
                            static_cast<double>(N) + static_cast<double>(N);
  gigaFlops =
      (flopsPerMatrixMul * 1.0e-9f) / (msecGPUTotal / 1000.0f);
  printf(
      "Performance= %.2f GFlop/s, Time= %.3f msec\n",
      gigaFlops, msecGPUTotal);
